...
class Ajax {
	...
	public function __construct() {
		add_action( 'wp_ajax_do_crawl_process', [ $this, 'do_crawl_process' ] );
		add_action( 'wp_ajax_nopriv_do_crawl_process', [ $this, 'do_crawl_process' ] );

		add_action( 'wp_ajax_stop_sync_crawling', [ $this, 'stop_sync_crawling' ] );
		add_action( 'wp_ajax_nopriv_stop_sync_crawling', [ $this, 'stop_sync_crawling' ] );
	}
		...
		if ( ! empty( $_POST ) ) {
			...
			try {
				...
				wp_send_json_success(
					"<p class='seocrawl_success'>" . __( 'Crawling is completed!', 'seo-crawl' ) . '</p>',
					200
				);
			} catch ( \Exception $e ) {
				wp_send_json_error(
			...
			wp_send_json_error(
				__( 'Something went wrong!', 'seo-crawl' ),
				200
			);
		}
